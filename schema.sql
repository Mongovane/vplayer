...
-- ============================================================================
-- Upload requests (non-owner submissions awaiting approval)
--
-- The cloud library is shared and its bytes cost the owner's storage, so a
-- member who joined by invite may ask for a song to be added but not add it
-- themselves. A PUT to /api/library/:id from a non-owner lands here instead of
-- ingesting; the owner approves, and only then are the bytes fetched.
--
-- Keyed by song id, not by request, because two people asking for the same
-- song is one decision, not two. A rejected row is kept so the same request
-- does not silently reappear at the top of the queue on the next tap.
-- ============================================================================

CREATE TABLE IF NOT EXISTS track_requests (
  id           TEXT PRIMARY KEY,       -- song id, same wire format as tracks.id
  member_id    TEXT NOT NULL,
