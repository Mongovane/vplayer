...
**暂停之后不能指望任何东西还会运行。** iOS 在音频停止后很快就冻结后台页面——
定时器停、fetch 停。所以缓冲余量的门槛从 30 秒降到 10 秒:凡是要准备好的,都得
在音频还在播的时候准备好。
...
两者都跑过反向验证:把每个 bug 重新塞回去,确认对应的检查真的会红。目前覆盖
9 类回归——`play()` 前插 await、handler 变 async、只预热前向、暂停不 re-warm、
`wasPlaying` 门槛、无条件预拉字节、预热误触 LRU、预拉不保留字节、淘汰不 revoke。

### 播放诊断

设置面板底部:`分享日志 / 复制 / 查看 / 清空`。手机上不需要 console。

引擎保留最近 200 条事件(时间、`visibilityState`、事件名、音源类型),而且
**跨页面重启保留**——这一点是必须的,不是锦上添花。要读的那些事件发生在锁屏
期间,而 iOS 会冻结后台页面、内存紧张时直接丢弃它;回到 app 是一次重新加载,
模块级数组活不过去。所以日志写进 localStorage,并且把**上一次会话**和本次分开
呈现——「上一次会话」就是要查的那一段。

写入策略:普通事件最多每 2 秒一次;`session:*`、`play:fail`、`media:error`、
`lifecycle:*` 立即同步写入。因为一个安排在两秒后执行的写入,在被冻结的页面上
永远不会发生。

日志里**时间戳出现断层而中间没有任何行,本身就是结论**——说明页面被冻结,
handler 根本没运行,那不是音频会话问题而是生命周期问题。

复现流程:清空 → 锁屏 → 复现 → 回到 app → 分享。iOS 上优先用分享而不是复制:
share sheet 比剪贴板可靠得多,而且能直接发给别人。查看用的是 `<textarea>` 而
不是 `<p>`,手机上全选比在段落里手动划选容易得多。
